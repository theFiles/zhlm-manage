...
public class ImageInfo
{
  ...
  private boolean checkGif() throws IOException {
    ...
    while (true) {
      int localBitsPerPixel, extensionType, n, blockType = read();
      switch (blockType) {
  ...
  private String readLine(StringBuffer sb) throws IOException {
    while (true) {
      int value = read();
      boolean finished = (value == -1 || value == 10);
      if (!finished) {
        sb.append((char)value);
      }
      if (finished) return sb.toString();
    }
  }
...
/* Location:              D:\file\project\tuozhanbao-manage\WEB-INF\classes\!\com.rogrand\core\image\ImageInfo.class
